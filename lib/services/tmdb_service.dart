...
class TMDBService extends ChangeNotifier {
  ...
  bool get hasMoreUpcomingMovies =>
      _upcomingMoviesPage < _upcomingMoviesTotalPages;
  ...
  Future<void> search(String query) async {
    ...
    if (!hasApiKey) {
      _searchResults = _getMockTrending().where((element) {
        final title = (element['title'] ?? element['name'] ?? '').toString();
        return title.toLowerCase().contains(query.toLowerCase()) ||
            _similarity(title, query) > 0.45;
      }).toList();
      notifyListeners();
      return;
    }
    ...
    _setLoading(true);
    try {
      final Map<int, Map<String, dynamic>> resultsById = {};

      Future<void> runQueryBatch(String q) async {
        final futures = <Future<http.Response>>[
          http.get(Uri.parse(
              '$_baseUrl/search/movie?api_key=${AppConfig.tmdbApiKey}&query=${Uri.encodeComponent(q)}&language=en-US&page=1&include_adult=false')),
          http.get(Uri.parse(
              '$_baseUrl/search/movie?api_key=${AppConfig.tmdbApiKey}&query=${Uri.encodeComponent(q)}&language=hi-IN&page=1&include_adult=false')),
          http.get(Uri.parse(
              '$_baseUrl/search/tv?api_key=${AppConfig.tmdbApiKey}&query=${Uri.encodeComponent(q)}&language=en-US&page=1&include_adult=false')),
          http.get(Uri.parse(
              '$_baseUrl/search/tv?api_key=${AppConfig.tmdbApiKey}&query=${Uri.encodeComponent(q)}&language=hi-IN&page=1&include_adult=false')),
          http.get(Uri.parse(
              '$_baseUrl/search/multi?api_key=${AppConfig.tmdbApiKey}&query=${Uri.encodeComponent(q)}&language=en-US&page=1&include_adult=false')),
        ];
        ...
        for (final response in responses) {
          ...
          for (final item in results) {
            ...
            mediaType ??= item.containsKey('first_air_date') ? 'tv' : 'movie';
            if (mediaType != 'movie' && mediaType != 'tv') continue;
            item['media_type'] = mediaType;
            resultsById.putIfAbsent(id, () => Map<String, dynamic>.from(item));
          }
        }
      }
      ...
      if (resultsById.length < 4) {
        final variants = _generateQueryVariants(query);
        if (variants.isNotEmpty) {
          await Future.wait(variants.map(runQueryBatch));
        }
      }

      await _searchByPerson(query, resultsById);

      final allResults = resultsById.values.toList();
      final queryLower = query.toLowerCase();
      allResults.sort((a, b) {
        double score(Map item) {
          final title = ((item['title'] ?? item['name'] ?? '') as String);
          final pop = (item['popularity'] as num?)?.toDouble() ?? 0.0;
          final sim = _similarity(title, query);
          final exactBonus = title.toLowerCase() == queryLower ? 100.0 : 0.0;
          final castBonus = item['_matchedVia'] != null ? 15.0 : 0.0;
  ...
  Future<void> _searchByPerson(
      String query,
      Map<int, Map<String, dynamic>> resultsById,
      ) async {
    try {
      final res = await http
          .get(Uri.parse(
          '$_baseUrl/search/person?api_key=${AppConfig.tmdbApiKey}&query=${Uri.encodeComponent(query)}&language=en-US&page=1&include_adult=false'))
          .timeout(const Duration(seconds: 10));
      if (res.statusCode != 200) return;
      final data = json.decode(res.body);
      final people = (data['results'] as List<dynamic>? ?? []);
      if (people.isEmpty) return;

      people.sort((a, b) {
        final simA = _similarity((a['name'] ?? '') as String, query);
        final simB = _similarity((b['name'] ?? '') as String, query);
        final popA = (a['popularity'] as num?)?.toDouble() ?? 0.0;
        final popB = (b['popularity'] as num?)?.toDouble() ?? 0.0;
        return (simB * 100 + popB).compareTo(simA * 100 + popA);
      });

      final topPeople = people.where((p) {
        return _similarity((p['name'] ?? '') as String, query) > 0.35;
      }).take(2).toList();
      ...
      for (final person in topPeople) {
        ...
        final creditsRes = await http
            .get(Uri.parse(
            '$_baseUrl/person/$personId/combined_credits?api_key=${AppConfig.tmdbApiKey}&language=en-US'))
            .timeout(const Duration(seconds: 10));
        if (creditsRes.statusCode != 200) continue;
        ...
        for (final credit in cast.take(15)) {
          final id = credit['id'] as int?;
          if (id == null) continue;
          var mediaType = credit['media_type'] as String?;
          mediaType ??= credit.containsKey('first_air_date') ? 'tv' : 'movie';
          if (mediaType != 'movie' && mediaType != 'tv') continue;

          if (resultsById.containsKey(id)) {
            resultsById[id]!['_matchedVia'] = personName;
          } else {
            final item = Map<String, dynamic>.from(credit);
            item['media_type'] = mediaType;
            item['_matchedVia'] = personName;
            resultsById[id] = item;
          }
        }
      }
    } catch (e) {
      debugPrint('Error searching TMDB by person: $e');
    }
  }

  static const Map<String, List<String>> _vowelSwaps = {
    'a': ['e', 'u', 'o'],
    'e': ['a', 'i'],
    'i': ['e', 'y'],
    'o': ['u', 'a'],
  ...
  List<String> _generateQueryVariants(String query) {
    ...
    for (var i = 0; i < lower.length; i++) {
      final char = lower[i];
      final swaps = _vowelSwaps[char];
      if (swaps != null) {
        for (final swap in swaps) {
          variants.add(lower.replaceRange(i, i + 1, swap));
        }
      }
    }

    final singled =
    lower.replaceAllMapped(RegExp(r'(.)\1+'), (m) => m.group(1)!);
    if (singled != lower) variants.add(singled);

    if (lower.isNotEmpty && !RegExp(r'[aeiou]$').hasMatch(lower)) {
      variants.add(lower + lower[lower.length - 1]);
    }
  ...
  static int _levenshtein(String s1, String s2) {
    final s1len = s1.length, s2len = s2.length;
    if (s1len == 0) return s2len;
    if (s2len == 0) return s1len;
    final costs = List<int>.generate(s2len + 1, (i) => i);
  ...
  Future<void> fetchTrending() async {
    ...
    // Load cache immediately so UI isn't blank while network request runs
    try {
    ...
    try {
      ...
      //  1  movies popular
      //  2  tv popular
      //  3  netflix movies   (provider 8,  watch_region=US required by TMDB)
      //  4  netflix tv       (provider 8)
      //  5  prime movies     (provider 9)
      //  6  prime tv         (provider 9)
      //  7  disney movies    (provider 337)
      //  8  disney tv        (provider 337)
      //  9  action movies    (genre 28)
      // 10  comedy movies    (genre 35)
      // 11  scifi movies     (genre 878)
      // 12  horror movies    (genre 27)
      // 13  upcoming movies
      // 14  bollywood movies (language hi)
      // 15  pakistani tv     (language ur)
      final responses = await Future.wait([
        // 0 — trending
        http
            .get(Uri.parse(
            '$_baseUrl/trending/all/day?api_key=${AppConfig.tmdbApiKey}'))
            .timeout(const Duration(seconds: 15)),
        // 1 — popular movies
        http
            .get(Uri.parse(
            '$_baseUrl/discover/movie?api_key=${AppConfig.tmdbApiKey}&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 2 — popular tv
        http
            .get(Uri.parse(
            '$_baseUrl/discover/tv?api_key=${AppConfig.tmdbApiKey}&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 3 — netflix movies
        // NOTE: watch_region is a TMDB API requirement for with_watch_providers
        // to activate the filter — without it the param is silently ignored.
        // Using US gives the broadest catalogue coverage globally.
        http
            .get(Uri.parse(
            '$_baseUrl/discover/movie?api_key=${AppConfig.tmdbApiKey}&with_watch_providers=8&watch_region=US&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 4 — netflix tv
        http
            .get(Uri.parse(
            '$_baseUrl/discover/tv?api_key=${AppConfig.tmdbApiKey}&with_watch_providers=8&watch_region=US&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 5 — prime movies
        http
            .get(Uri.parse(
            '$_baseUrl/discover/movie?api_key=${AppConfig.tmdbApiKey}&with_watch_providers=9&watch_region=US&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 6 — prime tv
        http
            .get(Uri.parse(
            '$_baseUrl/discover/tv?api_key=${AppConfig.tmdbApiKey}&with_watch_providers=9&watch_region=US&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 7 — disney movies
        http
            .get(Uri.parse(
            '$_baseUrl/discover/movie?api_key=${AppConfig.tmdbApiKey}&with_watch_providers=337&watch_region=US&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 8 — disney tv
        http
            .get(Uri.parse(
            '$_baseUrl/discover/tv?api_key=${AppConfig.tmdbApiKey}&with_watch_providers=337&watch_region=US&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 9 — action
        http
            .get(Uri.parse(
            '$_baseUrl/discover/movie?api_key=${AppConfig.tmdbApiKey}&with_genres=28&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 10 — comedy
        http
            .get(Uri.parse(
            '$_baseUrl/discover/movie?api_key=${AppConfig.tmdbApiKey}&with_genres=35&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 11 — scifi
        http
            .get(Uri.parse(
            '$_baseUrl/discover/movie?api_key=${AppConfig.tmdbApiKey}&with_genres=878&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 12 — horror
        http
            .get(Uri.parse(
            '$_baseUrl/discover/movie?api_key=${AppConfig.tmdbApiKey}&with_genres=27&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 13 — upcoming
        http
            .get(Uri.parse(
            '$_baseUrl/movie/upcoming?api_key=${AppConfig.tmdbApiKey}'))
            .timeout(const Duration(seconds: 15)),
        // 14 — bollywood
        http
            .get(Uri.parse(
            '$_baseUrl/discover/movie?api_key=${AppConfig.tmdbApiKey}&with_original_language=hi&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
        // 15 — pakistani
        http
            .get(Uri.parse(
            '$_baseUrl/discover/tv?api_key=${AppConfig.tmdbApiKey}&with_original_language=ur&sort_by=popularity.desc'))
            .timeout(const Duration(seconds: 15)),
      ]);
      ...
      if (responses[0].statusCode == 200) {
        ...
        _trending = (data['results'] as List<dynamic>? ?? [])
            .where((item) =>
        item['media_type'] == 'movie' || item['media_type'] == 'tv')
            .toList();
      }
      ...
      // 3 + 4 — netflix (movies + tv merged, sorted by popularity)
          {
        final List<dynamic> netflixMovies =
        responses[3].statusCode == 200
            ? (json.decode(responses[3].body)['results'] as List<dynamic>? ?? [])
            .map((item) { item['media_type'] = 'movie'; return item; })
            .toList()
            : [];
        final List<dynamic> netflixTv =
        responses[4].statusCode == 200
            ? (json.decode(responses[4].body)['results'] as List<dynamic>? ?? [])
            .map((item) { item['media_type'] = 'tv'; return item; })
            .toList()
            : [];
        _netflix = [...netflixMovies, ...netflixTv];
        _netflix.sort((a, b) {
          final pa = (a['popularity'] as num?)?.toDouble() ?? 0.0;
          final pb = (b['popularity'] as num?)?.toDouble() ?? 0.0;
          return pb.compareTo(pa);
        });
      }

      // 5 + 6 — prime (movies + tv merged)
          {
        final List<dynamic> primeMovies =
        responses[5].statusCode == 200
            ? (json.decode(responses[5].body)['results'] as List<dynamic>? ?? [])
            .map((item) { item['media_type'] = 'movie'; return item; })
            .toList()
            : [];
        final List<dynamic> primeTv =
        responses[6].statusCode == 200
            ? (json.decode(responses[6].body)['results'] as List<dynamic>? ?? [])
            .map((item) { item['media_type'] = 'tv'; return item; })
            .toList()
            : [];
        _prime = [...primeMovies, ...primeTv];
        _prime.sort((a, b) {
          final pa = (a['popularity'] as num?)?.toDouble() ?? 0.0;
          final pb = (b['popularity'] as num?)?.toDouble() ?? 0.0;
          return pb.compareTo(pa);
      ...
      // 7 + 8 — disney (movies + tv merged)
          {
        final List<dynamic> disneyMovies =
        responses[7].statusCode == 200
            ? (json.decode(responses[7].body)['results'] as List<dynamic>? ?? [])
            .map((item) { item['media_type'] = 'movie'; return item; })
            .toList()
            : [];
        final List<dynamic> disneyTv =
        responses[8].statusCode == 200
            ? (json.decode(responses[8].body)['results'] as List<dynamic>? ?? [])
            .map((item) { item['media_type'] = 'tv'; return item; })
            .toList()
            : [];
        _disney = [...disneyMovies, ...disneyTv];
        _disney.sort((a, b) {
          final pa = (a['popularity'] as num?)?.toDouble() ?? 0.0;
          final pb = (b['popularity'] as num?)?.toDouble() ?? 0.0;
          return pb.compareTo(pa);
      ...
      if (responses[9].statusCode == 200) {
        final data = json.decode(responses[9].body);
      ...
      if (responses[10].statusCode == 200) {
        final data = json.decode(responses[10].body);
      ...
      if (responses[11].statusCode == 200) {
        final data = json.decode(responses[11].body);
      ...
      // 12 — horror
      if (responses[12].statusCode == 200) {
        final data = json.decode(responses[12].body);
      ...
      if (responses[13].statusCode == 200) {
        final data = json.decode(responses[13].body);
        final results = data['results'] as List<dynamic>? ?? [];
      ...
      // 14 — bollywood
      if (responses[14].statusCode == 200) {
        final data = json.decode(responses[14].body);
      ...
      // 15 — pakistani
      if (responses[15].statusCode == 200) {
        final data = json.decode(responses[15].body);
  ...
  Future<void> fetchUpcomingMovies({bool loadMore = false}) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        _upcomingMoviesFeed.sort(
              (a, b) => (a['release_date'] as String? ?? '')
              .compareTo(b['release_date'] as String? ?? ''),
        );
  ...
  Future<void> fetchUpcomingTv({bool loadMore = false}) async {
    ...
    try {
      final res = await http
          .get(Uri.parse(
          '$_baseUrl/discover/tv?api_key=${AppConfig.tmdbApiKey}&first_air_date.gte=$todayStr&sort_by=first_air_date.asc&page=$nextPage'))
          .timeout(const Duration(seconds: 15));
      if (res.statusCode == 200) {
        final data = json.decode(res.body);
        ...
        _upcomingTvFeed.sort(
              (a, b) => (a['first_air_date'] as String? ?? '')
  ...
  Future<List<dynamic>> fetchRecommendations(int id, String mediaType) async {
    if (!hasApiKey) return _getMockTrending().take(4).toList();
    try {
      final res = await http
          .get(Uri.parse(
          '$_baseUrl/$mediaType/$id/recommendations?api_key=${AppConfig.tmdbApiKey}'))
          .timeout(const Duration(seconds: 15));
      if (res.statusCode == 200) {
        final data = json.decode(res.body);
  ...
  Future<Map<String, dynamic>?> fetchDetails(
    ...
    if (!hasApiKey) return _getMockDetails(id, mediaType, countryCode);
    try {
      final response = await http
          .get(Uri.parse(
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          debugPrint('Error parsing details for id=$id: $e\n$st');
          return null;
        }
      }
    } catch (e) {
  ...
  List<Map<String, String>> _mapProviders(List<dynamic> list) {
    return list.whereType<Map>().map((p) {
      ...
      final name =
      (rawName is String && rawName.isNotEmpty) ? rawName : 'Provider';
      return <String, String>{'provider_name': name, 'logo_path': logoPath};
    }).toList();
  }
  ...
  Map<String, dynamic> _parseDetails(
    ...
    (providersWrapper is Map<String, dynamic>)
        ? (providersWrapper['results'] ?? {})
        : {};
    ...
    final List<dynamic> freeProviders =
    regionMap['free'] is List ? regionMap['free'] : [];
    final List<dynamic> adsProviders =
    regionMap['ads'] is List ? regionMap['ads'] : [];
    final List<dynamic> subscriptionProviders =
    regionMap['flatrate'] is List ? regionMap['flatrate'] : [];
    final videosWrapper = data['videos'];
    final List<dynamic> videos = (videosWrapper is Map<String, dynamic>)
        ? (videosWrapper['results'] ?? [])
        : [];
    ...
    final List<dynamic> rawCastList = (creditsWrapper is Map<String, dynamic>)
        ? (creditsWrapper['cast'] ?? [])
        : [];
    final List<dynamic> castList = rawCastList
        .whereType<Map>()
    ...
    final List<dynamic> safeSeasons =
    (rawSeasons is List) ? rawSeasons : <dynamic>[];
    return {
      'id': data['id'],
  ...
  List<dynamic> _getMockMovies() => [
    {
      ...
      'media_type': 'movie',
      'poster_path':
      'https://images.unsplash.com/photo-1594909122845-11baa439b7bf?w=1000',
      'backdrop_path':
    ...
    },
    {
      ...
      'media_type': 'movie',
      'poster_path':
      'https://images.unsplash.com/photo-1478760329108-5c3ed9d495a0?w=1000',
      'backdrop_path':
  ...
  List<dynamic> _getMockSeries() => [
    {
      ...
      'media_type': 'tv',
      'poster_path':
      'https://images.unsplash.com/photo-1618336753974-aae8e04506aa?w=1000',
      'backdrop_path':
      'https://images.unsplash.com/photo-1618336753974-aae8e04506aa?w=1200',
      'vote_average': 8.6,
  ...
  List<dynamic> _getMockUpcoming() => [
    {
      ...
      'overview': 'The upcoming epic science fiction film...',
    },
    {
      'id': 102,
      ...
      'overview': 'An upcoming American superhero film...',
    },
  ];
  ...
  List<dynamic> _getMockUpcomingTv() => [
    {
      ...
      'media_type': 'tv',
      'poster_path':
      'https://images.unsplash.com/photo-1535016120720-40c646be5580?w=1000',
      'backdrop_path':
      'https://images.unsplash.com/photo-1535016120720-40c646be5580?w=1200',
      'vote_average': 0.0,
  ...
  Map<String, dynamic> _getMockDetails(
      int id,
      String mediaType,
      String countryCode,
      ) {
    final item = _getMockTrending().firstWhere(
          (e) => e['id'] == id,
      orElse: () => _getMockTrending().first,
    );
    String? trailerId;
      ...
      freeList = [
        {
          ...
          'logo_path': 'https://placehold.co/100x100/orange/white?text=Tubi',
        },
      ];
    } else if (id == 2) {
      ...
      freeList = [
        {
          ...
          'logo_path': 'https://placehold.co/100x100/red/white?text=YouTube',
        },
      ];
    } else if (id == 3) {
      ...
      freeList = [
        {
          ...
          'logo_path': 'https://placehold.co/100x100/yellow/black?text=Pluto',
        },
        {
          'provider_name': 'Tubi TV',
          'logo_path': 'https://placehold.co/100x100/orange/white?text=Tubi',
